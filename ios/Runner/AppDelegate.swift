...
class RtmpAudioPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "com.example.flutter_rtmp_audio_app/rtmp_audio",
        ...
        let eventChannel = FlutterEventChannel(
            name: "com.example.flutter_rtmp_audio_app/rtmp_audio_events",
            binaryMessenger: registrar.messenger())
    ...
    func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "startStreaming":
                ...
                let url = args["url"] as? String
            {
                ...
                result(
                    FlutterError(
                        code: "INVALID_ARGUMENTS", message: "URL is required", details: nil))
            }
    ...
    private func setupStreaming(url: String) {
        ...
        do {
            ...
            if let stream = rtmpStream {
                ...
                print(
                    "Audio stream configured with sampleRate: \(sampleRate), channels: \(channelCount), bitrate: \(bitrate)"
                )
            }
            ...
            rtmpConnection?.addEventListener(
                .rtmpStatus, selector: #selector(rtmpStatusHandler), observer: self)
            rtmpConnection?.addEventListener(
                .ioError, selector: #selector(rtmpErrorHandler), observer: self)
